...
int log_vprintf(int loglevel,char *fmt,va_list ap);
...
unsigned int fMIPS32_ExecuteInRom(unsigned int *pui_module,unsigned int start_address,unsigned int end_address, int forceacc);
...
#define EXEC_INSCOUNT 0x80
#define WAITACC 1
#define WAITDEBUG 2
unsigned int get_acc(unsigned int *op,int *address,unsigned long long **data);
unsigned long long *fJtag_getall(long long *pui_control, unsigned long long *pui_address, unsigned long long *pui_data);
...
int fJTAG_InstDatas(int wb, unsigned short *ir,drtype *dr, drtype *olddr);
unsigned char fJTAG_Instruction(unsigned char uc_data_out);
unsigned long long fJTAG_Data (unsigned long long ui_data_out);
...
int gdb_is_transmem(unsigned long long addr,unsigned int len);
int gdb_can_access(unsigned long long addr,unsigned int len);
int jump(int ret);
int jumperr(int ret, char *fmt, ...);
#define CMD_CPU "\1"
#define CMD_CPU1 "\2"
#define CMD_MEM "\3"
#define CMD_CACHE "\4"
