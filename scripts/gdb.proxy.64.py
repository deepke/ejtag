...
def getpgd(addr):
    if (addr&0xf000000000000000) == 0xc000000000000000:
        ...
        if 'module_pg_dir' in sym:
           curpgd=sym['module_pg_dir'];
        else :
           curpgd = sym['swapper_pg_dir']
    elif debugpgd:
        curpgd = debugpgd
...
def accesshelper(head,pgdaddr,tail,pgd,addr):
...
# 11 = (14-3)
# 12-2
        pud=read_addr8(sock,((pgdaddr>>((pageshift-3)*3))&pgdmask) + pgd);
	...
	if debuginfo: print("pgd=%x pud=%x pmd=%x pte=%x pte1=%x\n" % (pgd, pud, pmd, pte, pte1));
...
if  len(sys.argv)>=2:
    ...
    for x in ('swapper_pg_dir', 'pgd_current', 'module_kallsyms_lookup_name','module_pg_dir'):
        func[x] = 1
...
peeraddr = ('',50010)
if len(sys.argv)>2:
    addr = re.search(r'(.*):(\d+)',sys.argv[2]).groups()
    peeraddr = (addr[0],int(addr[1]))
                      ...
                      try:
                        if testre(re.search(r'\$(m|M)([^,]+)(,[^#]+)#',buf),m):
                            ...
                            if  ((addr>>32)&0xf0000000)==0xc0000000:
                                ...
                                buf = accesshelper(cmd,addr - 0xc000000000000000, tail,pgd, addr);
                            elif addr<0x8000000000000000:
                                buf = accesshelper(cmd, addr, tail, getpgd(addr), addr);
                            elif ((addr>>32)==0xffffffff and (addr&0xc0000000)==0xc0000000):
                                if 'module_pg_dir' in sym:
                                  pgd=sym['module_pg_dir'];
                                  buf = accesshelper(cmd, addr - 0xffffffffc0000000, tail, pgd, addr);
                                else :
                                  pgd=sym['swapper_pg_dir'];
                                  buf = accesshelper(cmd, addr, tail, pgd, addr);

                        elif enablesb and testre(re.search(r'\$z0,(\w+)(,\w+)#', buf),m):
                            addr=int(m[0].groups()[0],16)
                            tail=m[0].groups()[1];
                            if(((addr)&0xf0000000)==0xc0000000):
                                buf = accesshelper("z0,",addr, tail, getpgd(addr), addr)
                        elif enablesb and testre(re.search(r'\$Z0,(\w+)(,\w+)#',buf),m):
                            addr=int(m[0].groups()[0],16)
                            tail=m[0].groups()[1];
                            if(((addr)&0xf0000000)==0xc0000000):
